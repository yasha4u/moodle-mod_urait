...
namespace uraitservice_toolproxy\local\resources;
...
/**
 * Class toolproxy
 * @package uraitservice_toolproxy\local\resources
 */
class toolproxy extends \mod_urait\local\uraitservice\resource_base {

    /**
     * Class constructor.
     *
     * @param toolproxy $service Service instance
     */
    ...
    public function execute($response) {
        ...
        if ($ok && isset($toolproxyjson->security_contract->tool_service)) {
            ...
            $profileservice = '';
            $classname = "\\uraitservice_profile\\local\\service\\profile";
            if (class_exists($classname)) {
                $profileservice = new $classname();
            }
            $profileservice->set_tool_proxy($toolproxy);
            $path = (new \moodle_url('/mod/urait/services.php'))->out(false);
            $context = $path . $profileservice->get_resources()[0]->get_path() . '#';
