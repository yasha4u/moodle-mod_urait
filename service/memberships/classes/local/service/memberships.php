 ...
 *
 * @package uraitservice_memberships
 * @copyright 2024 LMS-Service {@link https://lms-service.ru/}
 * @author Daniil Romanov
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace uraitservice_memberships\local\service;

defined('MOODLE_INTERNAL') || die();

/**
 * Class memberships
 * @package uraitservice_memberships\local\service
 */
class memberships extends \mod_urait\local\uraitservice\service_base {
    ...
    public function get_resources() {
        ...
        if (empty($this->resources)) {
            ...
            $this->resources[] = new \uraitservice_memberships\local\resources\contextmemberships($this);
            $this->resources[] = new \uraitservice_memberships\local\resources\linkmemberships($this);
        }
     ...
     *
     * @param \mod_urait\local\uraitservice\resource_base $resource       Resource handling the request
    ...
    private function users_to_jsonld($resource, $users, $contextid, $exclude, $limitfrom, $limitnum,
        ...
        foreach ($users as $user) {
            ...
            $instanceconfig = null;
            if (!is_null($lti)) {
                $instanceconfig = urait_get_type_config_from_instance($lti->id);
            }
            ...
            if (!is_null($lti)) {
                ...
                $conditions = array('courseid' => $contextid, 'itemtype' => 'mod',
                        'itemmodule' => 'urait', 'iteminstance' => $lti->id);
    ...
    public function get_launch_parameters($messagetype, $courseid, $user, $typeid, $modlti = null) {
        ...
        $tool = urait_get_type_type_config($typeid);
        if (isset($tool->{$this->get_component_id()})) {
            $ok = $DB->record_exists('urait', array('course' => $courseid, 'typeid' => $typeid));
            $ok =  $ok || $DB->record_exists('lti_types', array('course' => $courseid, 'id' => $typeid));

            if ($tool->{$this->get_component_id()} == parent::SERVICE_ENABLED && $ok) {
