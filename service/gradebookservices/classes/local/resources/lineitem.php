...
namespace uraitservice_gradebookservices\local\resources;

use uraitservice_gradebookservices\local\service\gradebookservices;
...
/**
 * Class lineitem
 * @package uraitservice_gradebookservices\local\resources
 */
class lineitem extends resource_base {
     ...
     * @param \uraitservice_gradebookservices\local\resources\lineitem $olditem Grade item instance
     * @param string $typeid Tool Type Id
     *
     * @return string
     * @throws \Exception
    ...
    private function process_put_request($body, $olditem, $typeid) {
        ...
        if ($upgradegradebookservices) {
            if (is_null($typeid)) {
                ...
                $toolproxyid = null;
                $baseurl = urait_get_type_type_config($typeid)->lti_toolurl;
            }
     ...
     *
     * @param \uraitservice_gradebookservices\local\resources\lineitem $item Grade item instance
     * @throws \Exception
     */
    ...
    public function parse_value($value) {
        ...
        if (strpos($value, '$LineItem.url') !== false) {
            ...
            if (!empty($id)) {
                ...
                $id = $cm->instance;
                $item = grade_get_grades($COURSE->id, 'mod', 'urait', $id);
                if ($item && $item->items) {
