 ...
 *
 * @package uraitservice_gradebookservices
 * @copyright 2024 LMS-Service {@link https://lms-service.ru/}
 * @author Daniil Romanov
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace uraitservice_gradebookservices\local\service;

use uraitservice_gradebookservices\local\resources\lineitem;
use uraitservice_gradebookservices\local\resources\lineitems;
use uraitservice_gradebookservices\local\resources\results;
use uraitservice_gradebookservices\local\resources\scores;
...
/**
 * Class gradebookservices
 * @package uraitservice_gradebookservices\local\service
 */
class gradebookservices extends service_base {
                ...
                if ($this->is_allowed_in_context($typeid, $courseid)) {
                    ...
                    if (!is_null($modlti)) {
                        $conditions = array('courseid' => $courseid, 'itemtype' => 'mod',
                                'itemmodule' => 'urait', 'iteminstance' => $modlti);
    ...
    public function get_lineitems($courseid, $resourceid, $ltilinkid, $tag, $limitfrom, $limitnum, $typeid) {
        ...
        if ($lineitems) {
            foreach ($lineitems as $lineitem) {
                ...
                if ($gbs && (!isset($tag) || (isset($tag) && $gbs->tag == $tag))
                ...
                } else if (($lineitem->itemtype == 'mod' && $lineitem->itemmodule == 'urait'
                    ...
                    $ltiactivity = $DB->get_record('urait', array('id' => $lineitem->iteminstance));
     ...
     * @param string $typeid
     *
     * @return \uraitservice_gradebookservices\local\resources\lineitem|bool
     */
    ...
    public static function check_lti_id($linkid, $course, $toolproxy) {
        ...
        $ltiactivity = $DB->get_record('urait', array('id' => $linkid, 'course' => $course));
        if ($ltiactivity->typeid == 0) {
            ...
            $sql = 'SELECT urait.*
                      FROM {urait} urait
                INNER JOIN {lti_types} typ ON urait.typeid = typ.id
                     WHERE urait.id = ?
                           AND urait.course = ?
                           AND typ.toolproxyid = ?';
    ...
    public static function update_coupled_gradebookservices(object $ltiinstance,
        ...
        if ($ltiinstance && $ltiinstance->typeid) {
            $gradeitem = $DB->get_record('grade_items', array('itemmodule' => 'urait', 'iteminstance' => $ltiinstance->id));
            if ($gradeitem) {
                ...
                if ($gbs) {
                    ...
                    $baseurl = urait_get_type_type_config($ltiinstance->typeid)->lti_toolurl;
                    $DB->insert_record('ltiservice_gradebookservices', (object)array(
    ...
    public static function find_ltiservice_gradebookservice_for_lti($instanceid) {
        ...
        if ($instanceid) {
            $gradeitem = $DB->get_record('grade_items', array('itemmodule' => 'urait', 'iteminstance' => $instanceid));
            if ($gradeitem) {
