...
$config = urait_get_type_type_config($id);
if ($config->lti_ltiversion === LTI_VERSION_1P3) {
    if (!isset($SESSION->lti_initiatelogin_status)) {
        echo urait_initiate_login($courseid, 0, null, $config, 'ContentItemSelectionRequest', $title, $text);
        exit;
    } else {
...
// Prepare the request.
$request = urait_build_content_item_selection_request($id, $course, $returnurl, $title, $text, [], []);
